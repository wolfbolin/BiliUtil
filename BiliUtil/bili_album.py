...
class Album:
    ...
    def set_by_url(self, url):
        ...
        self.aid = re.match('/video/av([0-9]+)', input_url.path).group(1)
    ...
    def get_album(self, base_path='', name_path=False):
        if len(self.video) == 0:
            self.get_album_info()
        ...
        http_result = requests.get(self.pic)
        ...
        for video in self.video:
            video.get_video(cache_path, name_path)
